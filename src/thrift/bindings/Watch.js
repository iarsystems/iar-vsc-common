...
const WatchClient = class WatchClient extends ListWindowBackendClient {
  ...
  send_add (expr) {
    ...
    const args = new Watch_add_args(params);
    try {
  ...
  recv_add (input, mtype, rseqid) {
    ...
    const result = new Watch_add_result();
    result["read"](input);
    input.readMessageEnd();
...
export { WatchClient as Client };
const WatchProcessor = class WatchProcessor extends ListWindowBackendProcessor {
  constructor(handler) {
    super(handler);
  ...
  process_add (seqid, input, output) {
    const args = new Watch_add_args();
    args["read"](input);
    input.readMessageEnd();
    if (this._handler.add.length === 1) {
      new Promise((resolve) => resolve(this._handler.add.bind(this._handler)(
        args.expr
      ))).then(result => {
        const result_obj = new Watch_add_result({success: result});
      ...
      this._handler.add(args.expr, (err, result) => {
        let result_obj;
...
export { WatchProcessor as Processor };
